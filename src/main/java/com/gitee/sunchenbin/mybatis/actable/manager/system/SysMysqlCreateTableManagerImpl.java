...
public class SysMysqlCreateTableManagerImpl implements SysMysqlCreateTableManager {
	...
	public void createMysqlTable() {
		...
		// 不做任何事情
		if (!"none".equals(tableAuto) && !"update".equals(tableAuto) && !"create".equals(tableAuto) && !"add".equals(tableAuto)&& !"upforce".equals(tableAuto)) {
			log.warn("配置mybatis.table.auto错误无法识别，当前配置只支持[none/update/create/add/upforce]四种类型!");
			return;
		}
	...
	public void createMysqlTableApiPack(String mode,String packUrl) {
	...
	public void createMysqlTableApiObj(String mode,Class<?> clas) {
		// 读取配置信息
		//pack = packUrl;
		tableAuto = mode;
		...
		if (modifyTableMap.size() > 0) {
			for (Entry<String, TableConfig> entry : modifyTableMap.entrySet()) {
				for (Object obj : entry.getValue().getList()) {
					...
					if("upforce".equals(tableAuto))createMysqlTablesMapper.modifyTableFieldForce(map);
					//else createMysqlTablesMapper.modifyTableField(map);
